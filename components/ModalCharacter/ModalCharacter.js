            ...
            <table className={styles.table}>
                <tbody>
                {
                    Object.keys(tableData).map( article => (
                        <tr key={article}>
                            <td className={styles.col}>{article}:</td><td className={styles.col}>{tableData[article]}</td>
                        </tr>
                     ))
                }
                </tbody>
            </table>
            <button className={`${styles.close_button} fa-solid fa-xmark`} onClick={() => infoClickHandler(undefined)} arial-label='close'></button>
        </div>,
